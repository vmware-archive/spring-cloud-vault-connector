...
/**
 * {@link LocalConfigServiceInfoCreator} for {@link VaultServiceInfo} for property-based service
 * info creation.
 * <p>
 * {@link VaultServiceInfoCreator} creates {@link VaultServiceInfo} from a URI specifying scheme,
 * host, port and query parameters (token, backends).
 *
 * <h3>Query parameters</h3>
 * <ul>
 * <li>{@code token}: Required, authentication token.</li>
 * <li>{@code backend.ID}: Optional, dedicated backend path where ID is the backend Id (generic,
 * transit) and the value the path.</li>
 * <li>{@code shared_backend.ID}: Optional, shared backend path where ID is the shared backend Id
 * (organization, space) and the value the path.</li>
 * </ul>
 *
 * <h3>Example</h3>
...
public class VaultServiceInfoCreator extends
	...
	public VaultServiceInfo createServiceInfo(String id, String uri) {
		...
		Map<String, List<String>> backends = (Map<String, List<String>>) getBackends(queryParams, backendPattern);
		Map<String, String> sharedBackends = (Map<String, String>) getBackends(queryParams, sharedBackendPattern);

		return new VaultServiceInfo(id, address, token.toCharArray(), backends,
		        sharedBackends);
	}

	private Map<String, ?> getBackends(MultiValueMap<String, String> queryParams,
	        Pattern pattern) {
